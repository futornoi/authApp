...
const CurrentUserCard: React.FC<ICurrentUserCard> = ({
  user,
  isMe,
  role,
  deleteUser,
    ...
    <div className={`card userCard ${isMe ? 'isMeCard' : ''}`}>
      {role && (
        <MoreActionsPopup className="userCard-more">
          <button className="default-link edit" onClick={openEditMode}>
            Edit
          </button>
          <button className="default-link delete" onClick={deleteUser}>
            Delete
          </button>
        </MoreActionsPopup>
      )}
      ...
      <div className="user-data">
        ...
        <div className="user-field">
          <span>Role:</span> <strong>{user.roles}</strong>
        </div>
      </div>
    </div>
  );
