...
const Footer:React.FC<ILayout> = ({openedNav}) => {
  return (
    <footer id="footer" style={{transitionDelay: openedNav ? '' : '.2s'}} className={openedNav ? 'openedNav' : ''}>
      <Container>
        <div className="contacts">
          {contactOptions.map(({ Icon, href, target }, index) => (
            <a key={index} href={href} className="contacts_item" target={target ? '_blank' : ''}><Icon/></a>
          ))}
        </div>
      </Container>
    </footer>
